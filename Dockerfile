...
FROM kong/go-plugin-tool:2.0.4-alpine-latest AS build-go
COPY . /temp/pl
WORKDIR /temp/pl 
...
COPY --from=build-go /temp/pl/pep-plugin /kong/go-plugins/

RUN chmod -R 777 /kong/go-plugins/pep-plugin
...
RUN /kong/go-plugins/pep-plugin -dump
USER kong
